...
public class Painting : utility
{
    ...
    {
       // Debug.Log("set painting called, pos :"+pos);
        transform.position = pos;
        //mesh.transform.localScale = size;

       
    }
    //mesh.transform.localScale=new ve
    //Start is called before the first frame update
    //void Start()
    //{
    //    string apath = Application.dataPath + "\\paintings.json";
    //    //Debug.Log("path :"+ apath);
    //    string jstring = File.ReadAllText(apath);
    //    Painting_tab paintings = JsonUtility.FromJson<Painting_tab>("{\"tab\":" + jstring + "}");
    //    //Debug.Log("painting height tab json test :  "+paintings?.tab?[0].height);
    //}
